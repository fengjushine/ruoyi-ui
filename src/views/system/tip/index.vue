<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tipList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="1：使用，0：过期" align="center" prop="type" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        type: [
          { required: true, message: "1：使用，0：过期不能为空", trigger: "change" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除每日赠言编号为"' + ids + '"的数据项？').then(function() {
        return delTip(ids);
      }).then(() => {
        this.getList();
